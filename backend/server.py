...
async def import_endorsements_from_excel(
    ...
    try:
        ...
        # Send email notifications to HR and Admin users mapped to policies
        if success_count > 0:
            try:
                # Collect unique policy numbers from imported endorsements
                imported_policies = list(set(str(row.get('policy_number', '')).strip() for _, row in df.iterrows() if pd.notna(row.get('policy_number'))))
                
                # Find Admin emails
                admin_users = await db.users.find({"role": "Admin"}, {"_id": 0, "email": 1, "full_name": 1}).to_list(100)
                admin_emails = [u["email"] for u in admin_users if u.get("email")]
                
                # Find HR users assigned to these policies
                hr_assignments = await db.policy_assignments.find(
                    {"policy_number": {"$in": imported_policies}}, {"_id": 0, "hr_user_id": 1}
                ).to_list(500)
                hr_ids = list(set(a["hr_user_id"] for a in hr_assignments))
                hr_users = await db.users.find({"id": {"$in": hr_ids}}, {"_id": 0, "email": 1}).to_list(100) if hr_ids else []
                hr_emails = [u["email"] for u in hr_users if u.get("email")]
                
                all_emails = list(set(admin_emails + hr_emails))
                if all_emails:
                    import_subject = f"InsureHub — {success_count} Endorsements Imported via Excel"
                    import_body = f"""
                    <div style="font-family: Arial, sans-serif; max-width: 600px;">
                        <h2 style="color: #E05A47;">Endorsement Excel Import</h2>
                        <p><strong>{success_count}</strong> endorsements were imported successfully by <strong>{current_user.full_name}</strong>.</p>
...
def generate_financial_summary_pdf(
    ...
    elements.append(Spacer(1, 20))
    elements.append(Paragraph("This report is auto-generated by InsureHub — Aarogya Innovate Pvt Ltd. For queries, contact your insurance administrator.", note_style))
...
async def export_financial_summary(
    ...
    if send_email_flag and current_user.email:
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <br>
            <p style="color: #64748b; font-size: 12px;">This is an auto-generated report from InsureHub — Aarogya Innovate Pvt Ltd.</p>
        </div>
        """
        background_tasks.add_task(
            send_email_notification,
...
async def normalize_cd_ledger_policies(current_user: User = Depends(get_current_user)):
    ...
    for entry in all_entries:
        ...
        if entry_pn in canonical_pns:
            continue
        # Try case-insensitive exact match on policy_number
        matched = None
        for p in all_policies:
            if re.match(f"^{re.escape(entry_pn)}$", p["policy_number"], re.IGNORECASE):
                matched = p["policy_number"]
                break
        # Try full holder name match (both directions)
        if not matched:
            for p in all_policies:
                holder = p.get("policy_holder_name", "")
                if not holder:
                    continue
                # entry_pn contains holder name OR holder name contains entry_pn
                if re.search(re.escape(entry_pn), holder, re.IGNORECASE):
                    matched = p["policy_number"]
                    break
                if re.search(re.escape(holder), entry_pn, re.IGNORECASE):
                    matched = p["policy_number"]
                    break
        # Try word-level match: significant words from holder name or entry_pn
        if not matched:
            entry_words = [w for w in entry_pn.split() if len(w) >= 3 and w.lower() not in skip_words]
            for p in all_policies:
                holder = p.get("policy_holder_name", "")
                if not holder:
                    continue
                holder_words = [w for w in holder.split() if len(w) >= 3 and w.lower() not in skip_words]
                # Check if any significant entry word matches any holder word
                for ew in entry_words:
                    for hw in holder_words:
                        if ew.lower() == hw.lower():
                            matched = p["policy_number"]
                            break
                    if matched:
                        break
...
@api_router.get("/employee-directory/template")
async def download_member_upload_template(current_user: User = Depends(get_current_user)):
    """Download Excel template for uploading active members to Employee Directory"""
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    template_data = [
        {"Employee ID": "EMP001", "Member Name": "Rajesh Kumar", "Relationship Type": "Employee", "DOB": "1985-06-15", "Age": 41, "Gender": "Male",
         "Policy Number": "GMC001", "Mobile": "+919876543210", "Email": "rajesh@company.com", "Sum Insured": 500000, "Coverage Type": "Floater"},
        {"Employee ID": "EMP001", "Member Name": "Sunita Kumar", "Relationship Type": "Spouse", "DOB": "1988-03-22", "Age": 38, "Gender": "Female",
         "Policy Number": "GMC001", "Mobile": "", "Email": "", "Sum Insured": 500000, "Coverage Type": "Floater"},
        {"Employee ID": "EMP001", "Member Name": "Aarav Kumar", "Relationship Type": "Kids1", "DOB": "2015-09-10", "Age": 11, "Gender": "Male",
         "Policy Number": "GMC001", "Mobile": "", "Email": "", "Sum Insured": 500000, "Coverage Type": "Floater"},
        {"Employee ID": "EMP002", "Member Name": "Priya Patel", "Relationship Type": "Employee", "DOB": "1990-11-30", "Age": 35, "Gender": "Female",
         "Policy Number": "GMC001", "Mobile": "+919876543211", "Email": "priya@company.com", "Sum Insured": 300000, "Coverage Type": "Non-Floater"},
    ]
    df = pd.DataFrame(template_data)
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        inst = pd.DataFrame({
            'Field': ['Employee ID', 'Member Name', 'Relationship Type', 'DOB', 'Age', 'Gender', 'Policy Number', 'Mobile', 'Email', 'Sum Insured', 'Coverage Type'],
            'Required': ['Optional', 'YES', 'YES', 'Optional', 'Optional', 'Optional', 'YES', 'Optional', 'Optional', 'Optional', 'Optional'],
            'Notes': [
                'Same ID links family members', 'Full name', 'Employee/Spouse/Kids1/Kids2/Mother/Father',
                'YYYY-MM-DD (used for rate card lookup)', 'Auto-calculated from DOB if blank',
                'Male/Female/Other', 'Must match existing policy', 'With country code e.g. +919876543210',
                'Email address', 'Coverage amount', 'Floater or Non-Floater'
            ]
        })
        inst.to_excel(writer, index=False, sheet_name='Instructions')
    output.seek(0)
...
async def upload_active_members(
    ...
    for col in df.columns:
        cl = col.strip().lower().replace(" ", "_")
        if cl in ("employee_id", "emp_id", "employeeid"): col_map[col] = "employee_id"
        elif cl in ("member_name", "name", "full_name", "membername"): col_map[col] = "member_name"
        elif cl in ("relationship_type", "relationship", "relation"): col_map[col] = "relationship_type"
        elif cl in ("dob", "date_of_birth", "dateofbirth"): col_map[col] = "dob"
        elif cl == "age": col_map[col] = "age"
        elif cl == "gender": col_map[col] = "gender"
        elif cl in ("policy_number", "policy", "policynumber"): col_map[col] = "policy_number"
        elif cl in ("mobile", "phone", "employee_mobile"): col_map[col] = "mobile"
        elif cl in ("email", "employee_email"): col_map[col] = "email"
        elif cl in ("sum_insured", "suminsured", "sum insured"): col_map[col] = "sum_insured"
        elif cl in ("coverage_type", "coverage"): col_map[col] = "coverage_type"
    df = df.rename(columns=col_map)

    if "member_name" not in df.columns or "policy_number" not in df.columns:
        raise HTTPException(status_code=400, detail="Missing required columns: Member Name, Policy Number")
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if not member_name or member_name == "nan":
                errors.append({"row": idx + 2, "error": "Missing member name"}); error_count += 1; continue

            policy_number = str(row.get("policy_number", "")).strip()
            if not policy_number or policy_number == "nan":
                errors.append({"row": idx + 2, "error": "Missing policy number"}); error_count += 1; continue

            # Normalize policy_number
            import re as re_mod
            canonical = await db.policies.find_one({"policy_number": {"$regex": f"^{re_mod.escape(policy_number)}$", "$options": "i"}}, {"_id": 0, "policy_number": 1, "id": 1, "inception_date": 1, "expiry_date": 1, "annual_premium_per_life": 1, "premium": 1, "total_lives_covered": 1})
            if not canonical:
                errors.append({"row": idx + 2, "error": f"Policy {policy_number} not found"}); error_count += 1; continue
            policy_number = canonical["policy_number"]

            rel_str = str(row.get("relationship_type", "Employee")).strip()
            try:
            ...
            endorsement = Endorsement(
                policy_id=canonical.get("id", ""),
                policy_number=policy_number,
                employee_id=emp_id,
                member_name=member_name,
                dob=dob, age=age, gender=gender,
                relationship_type=rel,
                endorsement_type=EndorsementType.ADDITION,
                coverage_type=CoverageType(cov) if cov in ("Floater", "Non-Floater") else None,
                sum_insured=si,
                per_life_premium=premium,
                annual_premium_per_life=policy_ppl,
                endorsement_date=today,
                effective_date=today,
                days_from_inception=days_from,
                days_in_policy_year=days_in,
                remaining_days=remaining,
                prorata_premium=prorata,
                status=EndorsementStatus.PENDING,
                submitted_by=current_user.id,
                employee_email=email,
                employee_mobile=mobile,
                remarks="Uploaded via Employee Directory",
            )
            doc = endorsement.model_dump()
            doc['created_at'] = doc['created_at'].isoformat()
            await db.endorsements.insert_one(doc)
            ...
            errors.append({"row": idx + 2, "error": str(e)}); error_count += 1

    # Email notification
    if success_count > 0 and background_tasks:
        try:
            admin_users = await db.users.find({"role": "Admin"}, {"_id": 0, "email": 1}).to_list(50)
            emails = [u["email"] for u in admin_users if u.get("email")]
            if emails:
                body = f"""<div style="font-family: Arial; max-width: 600px;">
    ...
    return {"total_rows": len(df), "success_count": success_count, "error_count": error_count, "errors": errors[:20]}
...
async def download_rater(
    ...
    if format == "pdf":
        ...
        elements.append(Paragraph(f"Generated on {datetime.now(timezone.utc).strftime('%d %b %Y, %H:%M UTC')} — InsureHub, Aarogya Innovate Pvt Ltd", note))
        doc.build(elements)
        buffer.seek(0)
...
async def explain_policy_terms(
    ...
    try:
        ...
        chat = LlmChat(
            ...
            system_message="""You are InsureHub's insurance policy expert by Aarogya Innovate Pvt Ltd. You explain complex insurance terms in simple, easy-to-understand language for HR professionals and employees.
...
- Always mention Aarogya Innovate Pvt Ltd wellness add-ons when relevant
...
9. **Aarogya Innovate Pvt Ltd Wellness Add-ons** - Modern AI-powered wellness features like:
...
async def compare_policies(
    ...
    try:
        ...
        chat = LlmChat(
            ...
            system_message="""You are InsureHub's insurance benchmarking expert by Aarogya Innovate Pvt Ltd. You compare insurance policies objectively and help HR teams make informed decisions.
...
- Mention how Aarogya Innovate Pvt Ltd add-ons can enhance any policy"""
        ).with_model("openai", "gpt-4o-mini")
...
7. **Aarogya Innovate Pvt Ltd Enhancement** - How our wellness add-ons can complement the chosen policy
...
async def analyze_policy_pdf(
    ...
    try:
        ...
        chat = LlmChat(
            ...
            system_message="""You are InsureHub's insurance document analyst by Aarogya Innovate Pvt Ltd. You analyze policy documents and extract key terms, conditions, and important details in a structured format.
...
9. **Aarogya Innovate Pvt Ltd Recommendation** - How our wellness add-ons can enhance this policy
...
async def compare_uploaded_documents(
    ...
    try:
        chat = LlmChat(
            ...
            system_message="""You are InsureHub's senior insurance benchmarking expert by Aarogya Innovate Pvt Ltd. You compare insurance policies from uploaded documents and pre-loaded benchmarks. You provide objective analysis with actionable enhancement recommendations.
...
- Always suggest Aarogya Innovate Pvt Ltd wellness add-ons"""
        ).with_model("openai", "gpt-4o-mini")
...
- Areas where Aarogya Innovate Pvt Ltd AI Wellness Suite can fill gaps:
  * Preventive health checkup tracking
  * AI health risk assessment
  * Telemedicine integration
  * Mental wellness programs
...
{{
  ...
  "aarogya_assist_addons": [
    "How Aarogya Innovate Pvt Ltd AI Wellness Suite complements - point 1",
    "Point 2"
  ]
}}"""
...
async def download_benchmark_report(
    ...
    if addons:
        elements.append(Spacer(1, 3*mm))
        elements.append(Paragraph("Aarogya Innovate Pvt Ltd Wellness Add-ons", heading_style))
        for a in addons:
    ...
    elements.append(Paragraph("Generated by InsureHub | Aarogya Innovate Pvt Ltd | www.aarogya-tech.com", footer_style))
...
async def recommend_policy(
    ...
    try:
        chat = LlmChat(
            ...
            system_message="""You are InsureHub's senior insurance advisor by Aarogya Innovate Pvt Ltd. You recommend the best-fit insurance policies based on company profiles. You have deep knowledge of the Indian group insurance market.
...
- Always recommend Aarogya Innovate Pvt Ltd wellness add-ons as value enhancers
...
6. **Aarogya Innovate Pvt Ltd Wellness Suite** — How our add-on package complements the recommendation:
   - AI health risk assessment
...
async def seed_policy_benchmarks():
    ...
    benchmarks = [
        ...
        },
        {
            "id": str(uuid.uuid4()),
            "policy_type": "Group Health",
            "insurer_name": "Aarogya Innovate Pvt Ltd",
