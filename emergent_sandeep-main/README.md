...
```bash
# Login as HR
curl -X POST "https://claimsportal-1.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"username": "hr@test.com", "password": "password123"}'

# Get policies (with JWT token)
curl -X GET "https://claimsportal-1.preview.emergentagent.com/api/policies" \
  -H "Authorization: Bearer {YOUR_TOKEN}"

# Submit endorsement
curl -X POST "https://claimsportal-1.preview.emergentagent.com/api/endorsements" \
  -H "Authorization: Bearer {YOUR_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
...
### Frontend Testing
1. Open: https://claimsportal-1.preview.emergentagent.com
